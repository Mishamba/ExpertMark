...
# Util modules
ENV MODEL_FILE  user/target/user-1.0-SNAPSHOT.jar
...
ENTRYPOINT ["sh", "-c"]
CMD ["exec vertx run $CURRENCY_VERTICLE_NAME -cp $CURRENCY_VERTICLE_HOME/* -cluster | vertx run $SPY_VERTICLE_NAME -cp $SPY_CLASSPATH -cluster | vertx run $RECOMMENDATION_VERTICLE_NAME -cp $RECOMMENDATION_VERTICLE_CLASSPATH -cluster | vertx run $SECURITY_VERTICLE_NAME -cp $SECURITY_VERTICLE_CLASSPATH -cluster | vertx run $USER_VERTICLE_NAME -cp $USER_VERTICLE_CLASSPATH -cluster"]
CMD ["exec vertx run $FORECAST_VERTICLE_NAME -cp $FORECAST_DELPHI_QUIZ_VERTICLE_CLASSPATH"]
CMD ["exec vertx run $SPY_VERTICLE_NAME -cp $SPY_CLASSPATH"]
