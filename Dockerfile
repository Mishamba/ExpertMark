...
ENV FORECAST_DELPHI_QUIZ_VERTICLE_CLASSPATH "$FORECAST_DELPHI_QUIZ_VERTICLE_HOME/*.jar:$MODEL_HOME/*.jar:$USER_VERTICLE_HOME/*.jar:$EXPERT_STATISTIC_HOME/*.jar:$UTIL_HOME/:*.jar"
ENV SPY_CLASSPATH "$SPY_VERTICLE_HOME/*.jar:$UTIL_HOME/*.jar"
ENV RECOMMENDATION_VERTICLE_CLASSPATH "$RECOMMENDATION_VERTICLE_HOME/*.jar:$FORECAST_DELPHI_QUIZ_VERTICLE_HOME/*.jar:$USER_VERTICLE_HOME/*.jar:$UTIL_HOME/*.jar"
ENV SECURITY_VERTICLE_CLASSPATH "$SECURITY_VERTICLE_HOME/*.jar:$USER_VERTICLE_HOME/*.jar"
ENV USER_VERTICLE_CLASSPATH "$USER_VERTICLE_HOME/*.jar:$EXPERT_STATISTIC_HOME/*.jar:$MODEL_HOME/*.jar:$UTIL_HOME/*.jar"

# Launch the verticles
ENTRYPOINT ["sh", "-c"]
CMD ["exec vertx run $CURRENCY_VERTICLE_NAME -cp $CURRENCY_VERTICLE_HOME/*.jar -cluster | vertx run $FORECAST_VERTICLE_NAME -cp $FORECAST_DELPHI_QUIZ_VERTICLE_CLASSPATH -cluster | vertx run $SPY_VERTICLE_NAME -cp $SPY_CLASSPATH -cluster | vertx run $RECOMMENDATION_VERTICLE_NAME -cp $RECOMMENDATION_VERTICLE_CLASSPATH -cluster | vertx run $SECURITY_VERTICLE_NAME -cp $SECURITY_VERTICLE_CLASSPATH -cluster | vertx run $USER_VERTICLE_NAME -cp $USER_VERTICLE_CLASSPATH -cluster"]
